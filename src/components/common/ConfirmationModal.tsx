...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  const iconColor = type === 'warning' ? 'text-red-600' : 'text-green-600';
  const iconBgColor = type === 'warning' ? 'bg-red-100' : 'bg-green-100';
  ...
  return (
    ...
    >
      <div
        className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 transform transition-all duration-300 scale-100"
      ...
      >
        ...
        <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
          ...
          <button
            type="button"
            onClick={onCancel}
            className="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 sm:mt-0 sm:w-auto sm:text-sm"
          >
            ยกเลิก
          </button>
        </div>
