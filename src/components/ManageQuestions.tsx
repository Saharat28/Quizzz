...
import { ArrowLeft, Edit, Trash2, ChevronLeft, ChevronRight, Plus, Filter, Search, Loader2 } from 'lucide-react';
import { useQuizContext } from '../context/QuizContext';
import { useNotification } from '../context/NotificationContext';
import { FirebaseQuestion } from '../services/firebaseService';

// EditQuestionModal component (No changes)
const EditQuestionModal: React.FC<{
    ...
    const handleSave = async () => { setIsSaving(true); await onSave(question.id!, { text: editedQuestion.text, imageUrl: editedQuestion.imageUrl }); setIsSaving(false); onClose(); };
            ...
            <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-xl w-full max-w-2xl p-8">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <textarea value={editedQuestion.text} onChange={(e) => setEditedQuestion(prev => ({ ...prev, text: e.target.value }))} rows={4} className={`${formInputStyle} resize-none`} />
                    </div>
                    <div>
                        <label className={formLabelStyle}>URL รูปภาพ (ถ้ามี)</label>
                        <input type="text" value={editedQuestion.imageUrl || ''} onChange={(e) => setEditedQuestion(prev => ({ ...prev, imageUrl: e.target.value }))} className={formInputStyle} placeholder="https://example.com/image.png" />
                    </div>
                </div>
...
const ManageQuestions: React.FC = () => {
  ...
  const { questions, quizSets, deleteQuestion, deleteMultipleQuestions, updateQuestion } = useQuizContext();
  ...
  const filteredQuestions = useMemo(() => {
    ...
    if (searchTerm.trim() !== '') {
        questionsToFilter = questionsToFilter.filter(q => q.text.toLowerCase().includes(searchTerm.trim().toLowerCase()));
    }
  ...
  const handleSelectOne = (id: string) => {
    setSelectedQuestions(prev => prev.includes(id) ? prev.filter(selectedId => selectedId !== id) : [...prev, id]);
  };
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-2xl shadow-lg overflow-hidden">
        <div className="p-6 border-b border-gray-800 flex flex-wrap items-center justify-between gap-6">
            <div className='flex flex-wrap items-center gap-6'>
                <div className="flex items-center space-x-2">
                    ...
                    <select id="quizset-filter" value={quizSetFilter} onChange={(e) => { setQuizSetFilter(e.target.value); setCurrentPage(1); setSelectedQuestions([]); }} className="bg-gray-800 border border-gray-700 rounded-xl text-white px-4 py-2 focus:ring-2 focus:ring-red-500">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <input type="text" placeholder="ค้นหาคำถาม..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); setSelectedQuestions([]); }} className="bg-gray-800 border border-gray-700 rounded-xl text-white px-4 py-2 focus:ring-2 focus:ring-red-500"/>
                </div>
            </div>
            {selectedQuestions.length > 0 && (
                <button onClick={handleBulkDelete} className="flex items-center space-x-2 px-4 py-2 bg-red-800 text-red-100 rounded-xl hover:bg-red-700 transition-colors">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-800/50">
                <tr>
                  <th className="px-6 py-3 text-center w-12"><input type="checkbox" className="form-checkbox h-5 w-5 text-red-500 bg-gray-700 border-gray-600 rounded focus:ring-red-500" checked={paginatedQuestions.length > 0 && selectedQuestions.length === paginatedQuestions.length} onChange={handleSelectAll} title="เลือกทั้งหมดในหน้านี้" /></th>
            ...
            <tbody className="divide-y divide-gray-800">
                {paginatedQuestions.map((question) => (
                  <tr key={question.id} className={`transition-colors ${selectedQuestions.includes(question.id!) ? 'bg-red-900/20' : 'hover:bg-gray-800/50'}`}>
                    <td className="px-6 py-4 text-center"><input type="checkbox" className="form-checkbox h-5 w-5 text-red-500 bg-gray-700 border-gray-600 rounded focus:ring-red-500" checked={selectedQuestions.includes(question.id!)} onChange={() => handleSelectOne(question.id!)} /></td>
                    <td className="px-6 py-4"><p className="text-white line-clamp-2" title={question.text}>{question.text}</p></td>
        ...
        {totalPages > 1 && (
            <div className="p-4 border-t border-gray-800 flex items-center justify-between text-gray-400">
                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="flex items-center space-x-2 px-4 py-2 bg-gray-800 rounded-lg hover:bg-gray-700 disabled:opacity-50"><ChevronLeft className="w-4 h-4" /><span>ก่อนหน้า</span></button>
                <span>หน้า {currentPage} จาก {totalPages}</span>
                <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="flex items-center space-x-2 px-4 py-2 bg-gray-800 rounded-lg hover:bg-gray-700 disabled:opacity-50"><span>ถัดไป</span><ChevronRight className="w-4 h-4" /></button>
            </div>
        )}
      </div>
    </div>
